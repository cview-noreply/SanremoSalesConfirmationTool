...
from utils import (
    ...
    create_name, create_filename_anken
)
...
BASE_FOLDER = ROOT_FOLDER / '15_クライアント提出物' 
...
def store_documents():
    ...
    # 反響リストを取得
    hankyo_df = selectfile_to_df('証跡振り分け一覧.csv', CSV_FOLDER)
    ...
    # ファイルがなければテンプレートから作成（土台作り）
    integrated_filepath = RESULT_FOLDER / 'ドキュメント振り分け結果.xlsx'
    if not integrated_filepath.exists():
        shutil.copy(template_filepath, integrated_filepath)

    # Excel操作（新規・既存に関わらず共通）
    with xw.App(visible=False) as app:
        ...
        try:
            ...
            # 書き込み中にファイルが開いている等のエラー対策
            tmp_save_filepath = serial_filepath(RESULT_FOLDER, 'ドキュメント振り分け結果', '.xlsx')
