...
class SanremoApp:
    ...
    def create_tab1_sheets(self):
        ...
        self.make_btn(c, "【一括】全工程を実行", lambda: [
            create_sheets.make_folders(), create_sheets.create_sheets(),
            create_sheets.check_sheets_at_creation(), create_sheets.create_send_list(),
            create_sheets.create_send_jisseki()
        ], primary=True, task_name="案件管理シート作成 全工程")
        self.make_btn(c, "フォルダ作成", create_sheets.make_folders, task_name="案件管理シート作成 > フォルダ作成")
        self.make_btn(c, "シート生成", create_sheets.create_sheets, task_name="案件管理シート作成 > シート生成")
        self.make_btn(c, "整合性チェック", create_sheets.check_sheets_at_creation, task_name="案件管理シート作成 > 整合性チェック")
        self.make_btn(c, "送付対象リスト出力", create_sheets.create_send_list, task_name="案件管理シート作成 > 送付対象リスト出力")
    ...
    def create_tab2_check(self):
        c = self.create_tab_content("2. 受理チェック")
        self.make_btn(c, "【一括】全工程を実行", lambda: [
            check_sheets.make_folders(), check_sheets.check_sheets_on_receipt(),
            check_sheets.create_receive_jisseki(), check_sheets.create_mail_list(),
            check_sheets.make_input_data_hankyo()
        ], primary=True, task_name="受理チェック 全工程")
        ...
        self.make_btn(c, "回収実績CSV作成", check_sheets.create_receive_jisseki, task_name="受理チェック > 回収実績CSV作成")
    ...
    def create_tab3_list(self):
        c = self.create_tab_content("3. 送付リスト作成")
        self.make_btn(c, "案件管理シート一括送付用", create_sendlist.create_aks_bulk, task_name="送付リスト作成 > 案件管理シート一括送付用")
        self.make_btn(c, "成約/着工引き取り便作成", create_sendlist.create_doc_pickup, task_name="送付リスト作成 > 成約/着工引き取り便")
        self.make_btn(c, "個別着工証跡作成", create_sendlist.create_doc_pickup_indv, task_name="送付リスト作成 > 個別着工証跡")
        self.make_btn(c, "未提出リマインド作成", create_sendlist.create_aks_remind, task_name="送付リスト作成 > 未提出リマインド")
        self.make_btn(c, "HONEY進捗依頼作成", create_sendlist.create_Honey_progress, task_name="送付リスト作成 > HONEY進捗依頼")
        self.make_btn(c, "案件アラート一括送付用作成", create_sendlist.create_aa_bulk, task_name="送付リスト作成 > 案件アラート一括送付用")

    def create_tab4_store(self):
        c = self.create_tab_content("4. 証跡振り分け")
        self.make_btn(c, "振り分け実行", lambda: [store_documents.make_folders(), store_documents.store_documents()], primary=True, task_name="証跡振り分け 実行")
        self.make_btn(c, "フォルダ作成のみ", store_documents.make_folders, task_name="証跡振り分け > フォルダ作成")

    def create_tab5_alert(self):
        c = self.create_tab_content("5. 案件アラート作成")
        self.make_btn(c, "【一括】全工程を実行", lambda: [
            create_alert.make_folders(), create_alert.create_sheets(),
            create_alert.check_sheets_at_creation(), create_alert.make_send_list()
        ], primary=True, task_name="案件アラート作成 全工程")
        self.make_btn(c, "フォルダ作成", create_alert.make_folders, task_name="案件アラート作成 > フォルダ作成")
        self.make_btn(c, "アラート作成", create_alert.create_sheets, task_name="案件アラート作成 > アラート作成")
        self.make_btn(c, "作成後チェック", create_alert.check_sheets_at_creation, task_name="案件アラート作成 > 作成後チェック")
        self.make_btn(c, "送付対象一覧作成", create_alert.make_send_list, task_name="案件アラート作成 > 送付対象一覧作成")
